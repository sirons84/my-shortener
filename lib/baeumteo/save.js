...
/**
 * 다음 낱말을 실으려면 드는 카드. 사람 뽑는 값과 같은 비율로 오른다.
 *
 * 이 게임의 점수는 실은 낱말 수다. 그러니 값이 가파르게 올라야 등수가 갈린다.
 * 값이 그대로면 고용으로 수입만 곱절씩 늘어 카드가 남아돌고, 판은 낱말을
 * 누르는 속도 싸움이 된다. 사전을 다 채우는 것은 목표가 아니다.
 */
export function entryCost(config, loaded) {
  return Math.ceil(config.entry_cost * Math.pow(config.entry_growth ?? 1, loaded));
}

/** 뜻을 틀렸을 때 더 내는 카드. 낱말 값에 견주어 매긴다 (기획서 §8-1) */
export function wrongPenalty(config, loaded) {
  const ratio = config.wrong_penalty_ratio ?? 0;
  return Math.max(config.wrong_penalty ?? 0, Math.ceil(entryCost(config, loaded) * ratio));
}
