...
export function loadKeys() {
  ...
  try {
    ...
    return {
      ...
      // 말모이에 올린 낱말: { [id]: { key, rewarded } }
      malmoi: raw.malmoi && typeof raw.malmoi === 'object' ? raw.malmoi : {},
    };
  } catch {
    return empty();
  }
...
/** 말모이에 올린 낱말의 열쇠 */
export function rememberMalmoi(id, key) {
  const keys = loadKeys();
  keys.malmoi[id] = { key, rewarded: false };
  return write(keys);
}

/** 확인된 낱말의 카드를 받았다고 적어 둔다 (한 낱말에 한 번) */
export function rewardMalmoi(id) {
  const keys = loadKeys();
  if (keys.malmoi[id]) keys.malmoi[id] = { ...keys.malmoi[id], rewarded: true };
  return write(keys);
}

export function forgetMalmoi(id) {
  const keys = loadKeys();
  delete keys.malmoi[id];
  return write(keys);
}
