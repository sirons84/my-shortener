 ...
 */
// 개별 예외 계정: 배포 가능 개수를 별도로 지정 (소문자 이메일 → 개수)
const DROP_LIMIT_OVERRIDES = {
  'rlawngml1217@usedu.ai.kr': 10,
  'myjosephin@usedu.ai.kr': 10,
  'mossy33@usedu.ai.kr': 10,
  'gpdhkgus@naver.com': 10,
};

export function getDropLimit(email) {
  if (isAdmin(email)) return null; // 관리자는 무제한
  const key = (email || '').trim().toLowerCase();
  if (key in DROP_LIMIT_OVERRIDES) return DROP_LIMIT_OVERRIDES[key];
  if (key.endsWith('@usedu.ai.kr')) return 2;
  return 1;
}
